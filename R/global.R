...
globalStats <- function(g, glob, dir = FALSE) { 
  ...
  Assortativity = if('Assort'    %in% glob) assortativity.degree(g),
   Avg_Path_Len = if('Avg_Path'  %in% glob) average.path.length(g, dir, TRUE),
         Clique = if('Clique'    %in% glob) clique.number(g),
